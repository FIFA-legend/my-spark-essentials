...
object RDDs extends App {
  ...
  case class Movie(title: String, genre: String, rating: Option[Double])
  ...
  val moviesDS = moviesDF.select(
    col("Title").as("title"),
    col("Major_Genre").as("genre"),
    col("IMDB_Rating").as("rating")
  ).where(col("genre").isNotNull && col("rating").isNotNull).as[Movie]
  ...
  val goodDramaMoviesRDD = moviesRDD
    .filter(movie => movie.genre == "Drama" && movie.rating.getOrElse(0.toDouble) > 6)

  // 4
  val ratingByGenreRDD = moviesRDD.groupBy(_.genre)
    .map {
      case (genre, movies) =>
        (genre, movies.map(_.rating.getOrElse(0.toDouble)).sum / movies.size)
    }

  val ratingByGenreRDD2 = moviesRDD.groupBy(_.genre)
    .map {
      case (genre, movies) => (genre, movies.map(_.rating.getOrElse(0.toDouble)))
    }
    .map {
      case (genre, marks) => (genre, marks.sum / marks.size)
    }
  ...
  val avgRatingByGenreRDD = moviesRDD.groupBy(_.genre).map {
    case (genre, movies) => GenreAvgRating(genre, movies.map(_.rating.getOrElse(0.toDouble)).sum / movies.size)
  }
